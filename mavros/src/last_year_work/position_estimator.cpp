...
//---------------------------------------相关参数-----------------------------------------------
float dt;                                                  //程序运行的频率
int flag_position;                                         //0时不读取雷达数据，1时读取雷达数据
...
void optitrack_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
{
    ...
    pos_drone_mocap.position.x = msg->pose.position.y;
    pos_drone_mocap.position.y = msg->pose.position.x;
    pos_drone_mocap.position.z = - msg->pose.position.z;
    ...
    //optitrack系 到 NED系
    Euler_mocap[0] = Euler_mocap[0];
    Euler_mocap[1] = - Euler_mocap[1];
...
int main(int argc, char **argv)
{
    ...
    //读取参数表中的参数
    nh.param<int>("flag_position", flag_position, 1);
    ...
    // 【订阅】optitrack估计位置
    ros::Subscriber optitrack_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/M100/pose", 1000, optitrack_cb);

    // 【订阅】无人机当前位置 坐标系:NED系 [室外：GPS，室内：自主定位或mocap等] 这里作比较用
    ros::Subscriber position_sub = nh.subscribe<geometry_msgs::PoseStamped>("/mavros/local_position/pose", 10, pos_cb);

    // 【订阅】无人机当前速度 坐标系:NED系 [室外：GPS，室内：自主定位或mocap等] 这里作比较用
    ...
    tm* t = localtime(&tt);
    char iden_path[256];
    sprintf(iden_path, "/home/nuc/mavros3/data/position-estimator-%d-%02d-%02d_%02d-%02d.txt", t->tm_year+1900, t->tm_mon+1, t->tm_mday, t->tm_hour, t->tm_min);
...
void save_flight_data(std::ofstream& out_file, float timenow)
{
    out_file << timenow <<"  "<< pos_drone_mocap.position.x <<"  "<< pos_drone_mocap.position.y <<"  "<< pos_drone_mocap.position.z <<"  "\
                              ...
                              << Euler_laser[0] /PI*180<< Euler_laser[1] /PI*180 << Euler_laser[2] /PI*180<<" " \
...
void printf()
{
    ...
    cout <<">>>>>>>>>>>>>>>>>>>>>>>>>>>>Vins Info<<<<<<<<<<<<<<<<<<<<<<<<<" <<endl;
    cout << "Pos_vins: [X Y Z] : " << " " << fixed <<setprecision(5)<< vins.pose.pose.position.x << " [m] "<< vins.pose.pose.position.y<<" [m] "<< vins.pose.pose.position.z<<" [m] "<<endl;
    cout << "Vel_vins: [X Y Z] : " << " " << fixed <<setprecision(5)<< vins.twist.twist.linear.x << " [m/s] "<< vins.twist.twist.linear.y<<" [m/s] "<< vins.twist.twist.linear.z<<" [m/s] "<<endl;
