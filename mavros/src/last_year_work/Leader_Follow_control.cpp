...
double PI = 3.1415926;
double K1 = 0.75;
double K2 = 0.01;                                   //parameter of gain
double jing0 = 0;
double wei0 = 0;
int inter_time = 8;
int count_wpt = 0;
...
void printwaypoint(const mavros_msgs::WaypointList points)
{
    int count_waypoint;
    //############################
    cout<<"count_waypoint:"<<points.waypoints.size()<<endl;
...
int main(int argc, char **argv)
{
    ...
    //    while(ros::Time::now()-last_request<ros::DuAXIS MATPLOT EQUALration(5)){
    ...
    double home_latitude = 39.9889318;
    double home_longitude = 116.3514164;
...
//    }
    ...
    while(ros::ok())
    {
        ...
        //###########################
        while(ros::Time::now()-last_request<ros::Duration(T)){
        ...
        cout <<fixed<< setprecision(10)<< "leader_latitude[1]=" <<leader_latitude[1]<<endl;
        cout <<fixed<< setprecision(10)<< "leader_longitude[1]=" <<leader_longitude[1]<<endl;
        ...
        double duration ;
        cout <<"time_1-time_0="<<time_1-time_0<<endl;
        duration = T;
        cout <<"duration="<<duration<<endl;

        //cout <<fixed<< setprecision(10)<< "duration=" << duration <<endl;
        leader_Vx = (leader_x[1] - leader_x[0]) / duration;          //leader velocity
        leader_Vy = (leader_y[1] - leader_y[0]) / duration;
...
//        cout << "follower_y=" <<follower_y<<endl;
        ...
        cout << "desired_x=" <<desired_x<<endl;
        cout << "desired_y=" <<desired_y<<endl;
        ...
        //--------------------------------
        desired_V = K1*(leader_V) + K2 * distance;
        ...
        //current_latitude =  follower_latitude+(i + 1) * (desired_position_latitude-follower_latitude)/ inter_time;
        //current_longitude = follower_longitude + (i + 1)*(desired_position_longitude - follower_longitude)/ inter_time;
        //        waypoint[0].x_lat  = follower_latitude;
        //        waypoint[0].y_long = follower_longitude;
        //        waypoint[0].z_alt = H;
        ...
        //current_longitude = follower_longitude + (i + 1)*(desired_position_longitude - follower_longitude)/ inter_time;
        //#############################################
//        waypoint[2].x_lat  = desired_position_latitude;
//        waypoint[2].y_long = desired_position_longitude;
//        waypoint[2].z_alt = H;
...
//        waypoint[count_wpt].x_lat  = desired_position_latitude;
//        waypoint[count_wpt].y_long = desired_position_longitude;
//        waypoint[count_wpt].z_alt = H;
//        waypoint[count_wpt].command = mavros_msgs::CommandCode::NAV_WAYPOINT;
//        waypoint[count_wpt].frame = mavros_msgs::Waypoint::FRAME_GLOBAL_REL_ALT;
//        waypoint[count_wpt].autocontinue = true;

//        if(count_wpt == 0){
//            waypoint[count_wpt].is_current = true;
//        }
//        else{
//            waypoint[count_wpt].is_current = false;
//        }

//        count_wpt = count_wpt+1;

        //###################################################
        double current_latitude, current_longitude;
        //current_latitude =  follower_latitude+(i + 1) * (desired_position_latitude-follower_latitude)/ inter_time;
        //current_longitude = follower_longitude + (i + 1)*(desired_position_longitude - follower_longitude)/ inter_time;
        waypoint[0].x_lat  = home_latitude;
        waypoint[0].y_long = home_longitude;
        waypoint[0].z_alt = H;
        waypoint[0].command = mavros_msgs::CommandCode::NAV_WAYPOINT;
        waypoint[0].frame = mavros_msgs::Waypoint::FRAME_GLOBAL_REL_ALT;
        waypoint[0].autocontinue = true;
