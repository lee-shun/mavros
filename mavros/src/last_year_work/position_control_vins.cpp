...
void vins_cb(const nav_msgs::Odometry::ConstPtr& msg)
{
    ...
    pos_drone.position.x = msg->pose.pose.position.x;
    pos_drone.position.y = -msg->pose.pose.position.y;
    pos_drone.position.z = -msg->pose.pose.position.z;

    vel_drone.position.x = msg->twist.twist.linear.x;
    vel_drone.position.y = -msg->twist.twist.linear.y;
    vel_drone.position.z = -msg->twist.twist.linear.z;
    ...
    Vins_Euler[0] = Euler[0];
    Vins_Euler[1] = -Euler[1];
    Vins_Euler[2] = -Euler[2];

    euler_2_quaternion(Vins_Euler, q);

    mocap.pose.position.x = pos_drone.position.x;
    mocap.pose.position.y = pos_drone.position.y;
    mocap.pose.position.z = pos_drone.position.z;
...
{
    ros::init(argc, argv, "position_control_vins");
    ros::NodeHandle nh("~");
    ...
    // 【发布】无人机位置和偏航角 坐标系 NED系 [借助MOCAP的mavlink消息将外部测到的位置yaw角发布给飞控]
     mocap_pub = nh.advertise<geometry_msgs::PoseStamped>("/mavros/mocap/pose", 100);
...
void prinft_drone_state(float current_time)
{
    ...
    cout << "Attitude_vins : " << Vins_Euler[0] * 180/3.1415 <<" [°] "<<Vins_Euler[1] * 180/3.1415 << " [°] "<< Vins_Euler[2] * 180/3.1415<<" [°] "<<endl;
