...
//----------------
mavros_msgs::GlobalPositionTarget ps;
mavros_msgs::GlobalPositionTarget current_ps;
//-----------------------
...
void target_global_callback(const mavros_msgs::GlobalPositionTarget::ConstPtr& msg){
    current_ps = *msg;

    cout << "current_ps=" << current_ps <<endl<<endl;
}

void state_cb(const mavros_msgs::State::ConstPtr& msg){
    current_state = *msg;
    //cout << "current_state=" << current_state <<endl<<endl;
}
...
void uavLocalPoseReceived(const geometry_msgs::PoseStampedConstPtr& msg) //接受位置
{
    //uavPose = *msg;
...
{
    ros::init(argc, argv, "Controlor");
    ros::NodeHandle nh;
    //使用/mavros/setpoint_raw/global话题能够正确传送位置,但是会导致
    //无法进入offboard模式
    //使用/mavros/setpoint_position/global话题则是可以进入offboard模式
    //但是无法正确发送话题
    ros::Publisher pos_sp_pub1 = nh.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_position/global",10);
    ros::Publisher pos_sp_pub2 = nh.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_raw/global",10);

    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 10, state_cb);
    ros::Subscriber target_global_sub = nh.subscribe<mavros_msgs::GlobalPositionTarget>("mavros/setpoint_raw/target_global", 1000, target_global_callback);
...
//    //ros::Publisher vs_sp_pub = nh.advertise<geometry_msgs::TwistStamped>("/mavros/setpoint_velocity/cmd_vel", 100);
//    //ros::Publisher accel_sp_pub = nh.advertise<geometry_msgs::Vector3Stamped>("/mavros/setpoint_accel/accel", 100);
//    //global
//    ros::Publisher pos_sp_pub = nh.advertise<mavros_msgs::GlobalPositionTarget>("/mavros/setpoint_raw/global",1000);

//    //local 以home点为原点的北东地系(NED)
//    //ros::Publisher pos_sp_pub = nh.advertise<geometry_msgs::PoseStamped>("/mavros/setpoint_position/local",10);
//    ros::Subscriber state_sub = nh.subscribe<mavros_msgs::State>("mavros/state", 10, state_cb);
//    ros::Subscriber target_global_sub = nh.subscribe<mavros_msgs::GlobalPositionTarget>("mavros/setpoint_raw/target_global", 1000, target_global_callback);
...
//    //ros::Subscriber commandSubscriber = nh.subscribe<mavros::Key>("/keyboard/keydown", 1, sendCommand);
//    //ros::Subscriber uavLocalPoseSubscriber = nh.subscribe("/mavros/local_position/pose", 1000, uavLocalPoseReceived);
//    //ros::Subscriber uavLocalPoseSubscriber = nh.subscribe("/mavros/local_position/pose", 1000, uavLocalPoseReceived);
...
/**
        ...
        //                vs_sp_pub.publish(vs);


        //            vs_body_axis.twist.linear.x =
        //            vs_body_axis.twist.linear.y = vs.twist.linear.x * sin(uavYawENU) - vs.twist.linear.y * cos(uavYawENU);
        //            vs_body_axis.twist.linear.z = vs.twist.linear.z;

        //            accel.vector.x = 5.0;
        //            accel.vector.y = 0.0;
        //            accel.vector.z = 100.0;
        //            accel_sp_pub.publish(accel);
...
*/
        ...
        //位置控制
        ps.coordinate_frame = mavros_msgs::GlobalPositionTarget::FRAME_GLOBAL_INT;
        //ps.type_mask = mavros_msgs::GlobalPositionTarget::IGNORE_VX;
        ps.latitude = 39.9890091;
        ps.longitude = 116.3498186;
        ps.altitude = 50;
        ps.header.stamp = ros::Time::now();
        pos_sp_pub1.publish(ps);
        //pos_sp_pub1.publish(ps);
        //        }
        ros::spinOnce();
        loop_rate.sleep();
