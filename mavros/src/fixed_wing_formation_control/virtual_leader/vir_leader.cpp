 ...
 * 时间：2019.11.09
 */
#include "vir_leader.hpp" //记得修改一下路径

void _VIR_LEADER::write_to_files(string file_path_name, float time_stamp, float data)
{
...
float _VIR_LEADER::get_ros_time(ros::Time begin)
{
...
float _VIR_LEADER::get_air_speed(int type)
{
  if (type == 2)
  {
...
void _VIR_LEADER::update_rotmat()
{
...
void _VIR_LEADER::handle_status_from_receiver()
{
  //
}

void _VIR_LEADER::handle_message_from_px4()
{
  //
}

void _VIR_LEADER::send_the_command_to_px4()
{
  //
}

void _VIR_LEADER::send_message_to_sender(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
  ...
  fixed_wing_sub_pub_pointer->fixed_wing_states_tran.ned_vel_x = follower_status.ned_vel_x;
  fixed_wing_sub_pub_pointer->fixed_wing_states_tran.ned_vel_y = follower_status.ned_vel_y;
  fixed_wing_sub_pub_pointer->fixed_wing_states_tran.ned_vel_z = follower_status.ned_vel_z;

  fixed_wing_leader_state_pub.publish(fixed_wing_sub_pub_pointer->fixed_wing_states_tran);
}

void _VIR_LEADER::ros_sub_and_pub(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
  fixed_wing_states_sub                  //
      = nh.subscribe<mavros_msgs::State> //
        ("mavros/state", 10, &_FIXED_WING_SUB_PUB::state_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机imu信息，
  fixed_wing_imu_sub                   //
      = nh.subscribe<sensor_msgs::Imu> //
        ("mavros/imu/data", 10, &_FIXED_WING_SUB_PUB::imu_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机gps位置
  fixed_wing_global_position_form_px4_sub    //
      = nh.subscribe<sensor_msgs::NavSatFix> //
        ("mavros/global_position/global", 10, &_FIXED_WING_SUB_PUB::global_position_form_px4_cb, fixed_wing_sub_pub_pointer);

  //【订阅】无人机gps相对alt
  fixed_wing_global_rel_alt_from_px4_sub //
      = nh.subscribe<std_msgs::Float64>  //
        ("/mavros/global_position/rel_alt", 10, &_FIXED_WING_SUB_PUB::fixed_wing_global_rel_alt_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机ump位置
  fixed_wing_umt_position_from_px4_sub   //
      = nh.subscribe<nav_msgs::Odometry> //
        ("mavros/global_position/local", 10, &_FIXED_WING_SUB_PUB::umt_position_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机gps三向速度
  fixed_wing_velocity_global_fused_from_px4_sub   //
      = nh.subscribe<geometry_msgs::TwistStamped> //
        ("mavros/global_position/raw/gps_vel", 10, &_FIXED_WING_SUB_PUB::velocity_global_fused_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机ned位置
  fixed_wing_local_position_from_px4             //
      = nh.subscribe<geometry_msgs::PoseStamped> //
        ("mavros/local_position/pose", 10, &_FIXED_WING_SUB_PUB::local_position_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机ned三向速度
  fixed_wing_velocity_ned_fused_from_px4_sub      //
      = nh.subscribe<geometry_msgs::TwistStamped> //
        ("/mavros/local_position/velocity_local", 10, &_FIXED_WING_SUB_PUB::velocity_ned_fused_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机ned三向加速度
  fixed_wing_acc_ned_from_px4_sub                               //
      = nh.subscribe<geometry_msgs::AccelWithCovarianceStamped> //
        ("/mavros/local_position/accel", 10, &_FIXED_WING_SUB_PUB::acc_ned_from_px4_cb, fixed_wing_sub_pub_pointer);

  // 【订阅】无人机ned三向加速度
  fixed_wing_wind_estimate_from_px4_sub                         //
      = nh.subscribe<geometry_msgs::TwistWithCovarianceStamped> //
        ("/mavros/wind_estimation", 10, &_FIXED_WING_SUB_PUB::wind_estimate_from_px4_cb, fixed_wing_sub_pub_pointer);

  //                                  // 【订阅】无人机ned三向加速度
  fixed_wing_battrey_state_from_px4_sub         //
      = nh.subscribe<sensor_msgs::BatteryState> //
        ("/mavros/battery", 10, &_FIXED_WING_SUB_PUB::battrey_state_from_px4_cb, fixed_wing_sub_pub_pointer);

  fixed_wing_waypoints_sub                      //// 【订阅】无人机当前航点
      = nh.subscribe<mavros_msgs::WaypointList> //
        ("mavros/mission/waypoints", 10, &_FIXED_WING_SUB_PUB::waypointlist_from_px4_cb, fixed_wing_sub_pub_pointer);

  fixed_wing_waypointsreach_sub                    //// 【订阅】无人机到达的航点
      = nh.subscribe<mavros_msgs::WaypointReached> //
        ("mavros/mission/reached", 10, &_FIXED_WING_SUB_PUB::waypoints_reached_from_px4_cb, fixed_wing_sub_pub_pointer);

  fixed_wing_altitude_from_px4_sub          //订阅高度
      = nh.subscribe<mavros_msgs::Altitude> //
        ("/mavros/altitude", 10, &_FIXED_WING_SUB_PUB::altitude_from_px4_cb, fixed_wing_sub_pub_pointer);

  fixed_wing_air_ground_speed_from_px4_sub //订阅空速、地速
      = nh.subscribe<mavros_msgs::VFR_HUD> //
        ("/mavros/vfr_hud", 10, &_FIXED_WING_SUB_PUB::air_ground_speed_from_px4_cb, fixed_wing_sub_pub_pointer);

  //##########################################订阅消息###################################################//

  //##########################################发布消息###################################################//
  fixed_wing_leader_state_pub = // 发布读到到的leader的位置速度信息
      nh.advertise<mavros_msgs::Formation_fixed_wing>("/mavros/fixed_wing_formation/status", 10);
...
void _VIR_LEADER::control_formation()
{
  //
}
void _VIR_LEADER::trans_the_sp_for_send()
{
...
void _VIR_LEADER::send_setpoint_to_px4(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
...
void _VIR_LEADER::send_setpoint_to_ground_station()
{
  //
}

void _VIR_LEADER::test()
{
  //这里差分一下看看领机速度
...
bool _VIR_LEADER::set_fixed_wing_mode(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer, string setpoint_mode)
{
...
void _VIR_LEADER::show_fixed_wing_status(int PlaneID)
{
...
void _VIR_LEADER::show_fixed_wing_setpoint(int PlaneID)
{
...
void _VIR_LEADER::show_control_state(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
  for (int i = 1; i <= the_space_between_blocks; i++)
    cout << endl;
...
bool _VIR_LEADER::update_leader_status(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
  //空速
...
bool _VIR_LEADER::update_follwer_status(_FIXED_WING_SUB_PUB *fixed_wing_sub_pub_pointer)
{
...
void _VIR_LEADER::foramtion_demands_update(int formation_type)
{
...
void _VIR_LEADER::calculate_follower_error()
{
...
void _VIR_LEADER::show_formation_error(int PlaneID)
{
...
void _VIR_LEADER::control_vertical(float current_time)
{
  //不同模式切换的时候需要进行重置tecs
...
void _VIR_LEADER::show_tecs_status()
{
  for (int i = 1; i <= the_space_between_blocks; i++)
    cout << endl;
...
void _VIR_LEADER::control_lateral(float current_time)
{
...
void _VIR_LEADER::run(int argc, char **argv)
{
  ...
  while (ros::ok())
  {
    ...
    show_fixed_wing_status(planeID);
